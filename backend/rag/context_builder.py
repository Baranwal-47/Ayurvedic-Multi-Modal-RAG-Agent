...
class ContextBuilder:
    ...
    def build(self, query_bundle: QueryBundle, evidence: EvidenceSelection) -> ContextPack:
        citation_candidates = list(evidence.citation_candidates)
        citations: list[dict[str, Any]] = []
        ...
        for index, candidate in enumerate(evidence.image_candidates, start=1):
            card = self._make_image_card(candidate, card_id=f"F{index}", citation_id_map=citation_id_map)
            if card:
        ...
        enough_evidence = self._enough_evidence(evidence, citations, image_cards, table_cards)
